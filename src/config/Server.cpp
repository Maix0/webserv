...
/*   Updated: 2025/02/28 16:48:54 by maiboyer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Server Server::fromTomlValue(const TomlValue& toml) {
			...
			if (it->second.isInt()) {
				...
				for (TomlList::const_iterator it = l.begin(); it != l.end(); it++) {
					...
					if (std::find(out.ports.begin(), out.ports.end(), up) != out.ports.end())
						std::cerr << "Duplicate port in server (skipping...): " << up << std::endl;
					else
						out.ports.push_back(up);
				}
			}
	...
	if (seen.count("root") == 0)
		throw std::runtime_error("Missing key \"root\" in server");
	if (seen.count("port") == 0)
		throw std::runtime_error("Missing key \"ports\" in server");
